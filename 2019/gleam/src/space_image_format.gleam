...
fn split_by(count, list, acc) {
  case list {
    [] -> e_lists_reverse(acc)
    _ -> {
      let tuple(layer, rest) = e_lists_split(count, list)
...
pub fn main(_) {
  ...
  let layers =
    ...
    |> e_lists_map(fn(digit_as_byte) {
        let tuple(digit, _) = e_string_to_integer([digit_as_byte])
        digit
      }, _)
    |> split_by(25 * 6, _, [])

  let tuple(_, ones, twos) =
    layers
    |> e_lists_map(fn(layer) {
        e_lists_foldl(fn(d, acc) {
          let tuple(zeros, ones, twos) = acc
          case d {
            0 -> tuple(zeros + 1, ones, twos)
            1 -> tuple(zeros, ones + 1, twos)
            2 -> tuple(zeros, ones, twos + 1)
            _ -> tuple(zeros, ones, twos)
          }
        },
        tuple(0, 0, 0),
        layer)
