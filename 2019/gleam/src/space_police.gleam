...
external fn e_maps_to_list(map: Map(k, v)) -> List(tuple(k, v)) = "maps" "to_list"
...
fn reset_outputs(program_state) {
  let tuple(a, b, c, d, e, _) = program_state
  tuple(a, b, c, d, e, [])
}

fn add_inputs(program_state, new_inputs) {
  let tuple(a, b, c, d, e, f) = program_state
  tuple(a, b, c, d, e_lists_append([e, new_inputs]), f)
}
...
fn move(current_panel, current_direction, turn_direction) {
  ...
  tuple(new_panel, new_direction)
}

fn paint(program_state, area, current_panel, current_direction) {
  let tuple(_, _, _, _, _, outputs) = program_state
  case outputs {
    [] -> {
      area
    }
    [turn_direction, color_to_paint] -> {
      let tuple(new_panel, new_direction) = move(current_panel, current_direction, turn_direction)
      let new_program = 
        program_state
        |> reset_outputs(_)
...
pub fn main(_) {
  ...
  let painted_panels_count =
    paint(run_program(tuple(Unknown, software, 0, 0, [0], [])), e_maps_new(), tuple(0, 0), Up)
    |> e_maps_to_list(_)
    |> e_length(_)
  ...
  let area =
    paint(run_program(tuple(Unknown, software, 0, 0, [1], [])), e_maps_new(), tuple(0, 0), Up)
  ...
  let tuple(tuple(left_edge, _), _) = e_lists_min(area_list)
  let tuple(tuple(right_edge, _), _) = e_lists_max(area_list)
  let ys =
    area_list
    |> e_lists_map(
      fn(point) {
        let tuple(tuple(_, y), _) = point
        y
      },
      _
    )
  ...
  |> e_lists_map(
    fn(y) {
      ...
      |> e_lists_map(
        fn(x) {
          case e_maps_get_with_default(tuple(x, y), area, 0) {
            0 -> e_io_put_chars("#")
            1 -> e_io_put_chars(" ")
          }
          case x == right_edge {
